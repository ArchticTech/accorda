 ...
 */
export const createLoanRequest = async (customerId: string, formData: LoanApplicationFormData) => {
  try {
    // Start a transaction
 ...
 */
export const formatLoanRequestForDisplay = (loanRequest: LoanRequestWithDetails) => {
  // Map pay frequency to display text
  const payFrequencyMap = {
    '1month': 'Once a month',
    '2weeks': 'Every 2 weeks',
  ...
  return {
    ...
    payFrequency: payFrequencyMap[loanRequest.loanRequest.pay_frequency] || loanRequest.loanRequest.pay_frequency,
    reference: loanRequest.references.length > 0 ? loanRequest.references[0].name : 'N/A',
    ...
    incomeSource: {
      ...
      payFrequency: payFrequencyMap[loanRequest.loanRequest.pay_frequency] || loanRequest.loanRequest.pay_frequency,
      nextPayDate: loanRequest.loanRequest.next_pay_date,
      consumerProposal: loanRequest.loanRequest.consumer_proposal ? 'yes' : 'no',
    ...
    documents: loanRequest.documents ? loanRequest.documents.map(doc => ({
      id: doc.id,
