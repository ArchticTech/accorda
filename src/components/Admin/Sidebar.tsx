...
import { LayoutDashboard, FileText, PlusCircle, User, X, Users } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ closeSidebar }) => {
  ...
  const navItems = [
    ...
    { name: 'Requests', path: '/admin/requests', icon: <FileText className="w-5 h-5 mr-3" /> },
    { name: 'Loans', path: '/admin/loans', icon: <FileText className="w-5 h-5 mr-3" /> },
    { name: 'Perception', path: '/admin/perception', icon: <PlusCircle className="w-5 h-5 mr-3" /> },
    { name: 'Profile', path: '/admin/profile', icon: <User className="w-5 h-5 mr-3" /> },
    { name: 'Customers', path: '/admin/customers', icon: <Users className="w-5 h-5 mr-3" /> },
  ];
