...
serve(async (req) => {
  ...
  try {
    ...
    const { requestId, stage } = await req.json();

    if (!requestId || !stage) {
      return new Response(
        JSON.stringify({ success: false, error: "Missing requestId or stage" }),
    ...
    if (loanRequestError) {
      console.error("Error updating loan request stage:", loanRequestError);
      return new Response(
